...
program.command('address')
  ...
  .action(async (address, options) => {
    try {
      ...
      qrcode.generate(result.data?.address, { });
...
program.command('set')
  ...
  .option('--satsbyte <number>', 'Satoshis per byte in fees', '10')
...
program.command('seal')
  ...
  .option('--address <string>', 'Address to put the final output')
  .option('--funding <string>', 'Use wallet alias wif key to be used for funding')
  .option('--owner <string>', 'Use wallet alias WIF key to seal the Atomical')
  .option('--satsbyte <number>', 'Satoshis per byte in fees', '10')
  .option('--satsoutput <number>', 'Satoshis to put into the final output', '546')
  .action(async (atomicalAliasOrId, options) => {
    try {
      const walletInfo = await validateWalletStorage();
      const config: ConfigurationInterface = validateCliInputs();
      ...
      const receive: { output: any, address: string } = performAddressAliasReplacement(walletInfo, options.address || undefined);
      let ownerWalletRecord = resolveWalletAliasNew(walletInfo, options.owner, walletInfo.primary);
      const modifiedStrippedHash = atomicalAliasOrId.indexOf('#') === 0 ? atomicalAliasOrId.substring(1) : atomicalAliasOrId;
      const result: any = await atomicals.sealInteractive(modifiedStrippedHash, fundingWalletRecord, ownerWalletRecord, satsbyte, satsoutput, receive?.address);
      handleResultLogging(result);
    } catch (error) {
      console.log(error);
    }
